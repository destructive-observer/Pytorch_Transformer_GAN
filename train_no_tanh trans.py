...
class PGGAN():
    def __init__(self, G, D, data, noise, opts):
        ...
        self.bs_map=  {4: 256, 8: 128, 16: 64, 32: 32, 64: 16, 128: 8, 256:4}
        # self.rows_map = {32: 8, 16: 4, 8: 4, 4: 2, 2: 2}
        self.rows_map = {256: 8, 128: 8, 64: 8, 32: 4, 16: 4,8:4,4:4,2:4}
        # save opts
    ...
    def update_lr(self, cur_nimg):
        for param_group in self.optim_G.param_groups:
            ...
            param_group['lr'] = 1 * self.opts['g_lr_max']
        for param_group in self.optim_D.param_groups:
            # lrate_coef = self._rampup(cur_nimg / 1000.0, self.opts['rampup_kimg'])
            # lrate_coef *= self._rampdown_linear(cur_nimg / 1000.0, self.opts['total_kimg'], self.opts['rampdown_kimg'])
            param_group['lr'] = (0.8 **(cur_nimg-1)) * self.opts['d_lr_max']
    ...
    def preprocess(self, z, real,x_gt):
    ...
    def train(self):
        ...
        for R in range(from_level-1, to_level-1):
            ...
            for it in range(_num_it):
                ...
                x,x_gt = self.data(batch_size, cur_resol, cur_level)

                # preprocess
                self.preprocess(z, x,x_gt)
                # update D
                ...
                # sampling
                samples = []
                if (it % self.opts['sample_freq'] == 0) or it == _num_it-1 or abs(self.g_loss)>500:
                    samples = self.sample('')
                    imageio.imsave(os.path.join(self.opts['sample_dir'],
                                        '%dx%d-%s-%s.png' % (cur_resol, cur_resol, phase, str(it).zfill(6))), samples)

                # ===tensorboard visualization===
                if (it % self.opts['sample_freq'] == 0) or it == _num_it - 1 or abs(self.g_loss)>50000:
                    self.tensorboard(it+tensorboard_lt, _num_it, phase, cur_resol, samples)

                # save model
                if (it % self.opts['save_freq'] == 0 and it > 0) or it == _num_it-1 or abs(self.g_loss)>50000:
