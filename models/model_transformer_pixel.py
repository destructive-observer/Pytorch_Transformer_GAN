...
import math
from position_encoding import FourierMapping
...
class Fourier_encoding(nn.Module):
    def __init__(self, channel, dim):
        super().__init__()
        self.position = FourierMapping(dim,dim)
...
class Generator(nn.Module):
    def __init__(self, 
        ...
        layers += [nn.Linear(latent_size, 512)]
        # layers +=[nn.LeakyReLU(negative_slope=0.2)]
        layers += [nn.Linear(512, 768)]
        layers += [nn.Linear(768, 1024)]
        # layers +=[nn.LeakyReLU(negative_slope=0.2)]
